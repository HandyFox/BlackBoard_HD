...
Global $PDF_AUTHOR = "BlackBoard HD"
Global $PDF_CREATOR = "BlackBoard HD"
Global $PDF_COPYRIGHT = "© 1973-" & @YEAR & " Mihai Iancu 'MIPDF' Autoit PDF Library"
Global $PDF_TITLE = ""
Global $PDF_SUBJECT = ""
Global $PDF_KEYWORDS = ""
Global $PDF_NAME = ""
...
Func _ABC_Make_PDF($ABC_MakePDF_Dir, $ABC_PDF_File_Name)
   Local $i = 0
   $_Pages = 0
	Local $aImgs = _FileListToArray($ABC_MakePDF_Dir, "*.png")

		;set the properties for the pdf
		_SetTitle("BlackBoard HD Graph Report")
		_SetSubject("BH Graph")
		_SetKeywords("pdf, BlackBoard HD")
		_OpenAfter(True);open after generation
		_SetUnit($PDF_UNIT_CM)
		_SetPaperSize("letter")
		_SetZoomMode($PDF_ZOOM_CUSTOM,90)
		_SetOrientation($PDF_ORIENTATION_PORTRAIT)
		_SetLayoutMode($PDF_LAYOUT_CONTINOUS)

		;initialize the pdf
		_InitPDF($ABC_PDF_File_Name)

		If UBound($aImgs)<>1 Then
			;=== load resources used in pdf ===
			For $i=1 To $aImgs[0]
				_LoadResImage("img"&$i, $ABC_MakePDF_Dir&"\" & $aImgs[$i])
			 Next
			 $i = 0
			...
			For $i=1 To $aImgs[0]
				_BeginPage()
					...
					_InsertImage("img"&$i, 0.65, 8, 715/(_GetUnit()+7), 533/(_GetUnit()+7))
				_EndPage()
			Next
		 EndIf
